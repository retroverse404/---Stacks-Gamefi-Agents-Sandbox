...
(define-non-fungible-token wax-cylinder uint)
...
(define-private (token-exists (token-id uint))
  (is-some (nft-get-owner? wax-cylinder token-id))
)
...
(define-public (mint (recipient principal) (token-uri (optional (string-ascii 256))))
  ...
  (let
    ...
    (begin
      (try! (nft-mint? wax-cylinder next-token-id recipient))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  ...
  (begin
    (try! (nft-transfer? wax-cylinder token-id sender recipient))
    (print {
      event: "wax-cylinder-transferred",
      token-id: token-id,
      sender: sender,
      recipient: recipient
    })
...
(define-read-only (get-owner (token-id uint))
  ...
  (ok (nft-get-owner? wax-cylinder token-id))
)
